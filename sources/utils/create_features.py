...
from sources.transformerclassifier import TransformerClassifier, ConvNeXT
...
def add_features(dataframe: pd.DataFrame):
    ...
    with open(TEXT_FILE_PATH, 'a') as file:
        for index, row in dataframe.iterrows():
            ...
            if success:
                euclidean_similarity = euclidean_distance(classifier_embeddings[0], classifier_embeddings[1])
                cosine_similarity = cosine_distance(classifier_embeddings[0], classifier_embeddings[1])
...
if __name__ == "__main__":
    TEXT_FILE_PATH = '../../data/train_features(vit).txt'
    CSV_FILE_PATH = "../../data/train_features(vit).csv"
    #main()
    dataframe_from_txt()
